...
from databricks_tools_core.unity_catalog import (
    ...
    create_table_as_select as _create_table_as_select,
    clone_table as _clone_table,
    alter_table as _alter_table,
    # Grants
...
# =============================================================================
# Tool 2: manage_uc_tables
# =============================================================================


@mcp.tool
def manage_uc_tables(
    action: str,
    catalog_name: str = None,
    schema_name: str = None,
    table_name: str = None,
    ...
    new_name: str = None,
    owner: str = None,
    set_properties: Dict[str, str] = None,
    add_columns: List[Dict[str, str]] = None,
    warehouse_id: str = None,
) -> Dict[str, Any]:
    """
    Manage Unity Catalog tables with advanced DDL operations.

    Actions:
    - create: Create a table with columns. Supports liquid clustering (cluster_by) and table properties.
    - get: Get table details.
    - list: List tables in a schema.
    - delete: Delete a table.
    - create_as_select: Create table from a SELECT query (CTAS).
    - clone: Clone a table (deep or shallow, with optional time travel).
    - alter: Alter table (rename, change owner, set properties, add columns).

    Args:
        action: "create", "get", "list", "delete", "create_as_select", "clone", "alter"
        catalog_name: Catalog name (for create/list/clone/create_as_select)
        schema_name: Schema name (for create/list/clone/create_as_select)
        table_name: Table name (for create/clone/create_as_select)
        full_table_name: Full name catalog.schema.table (for get/delete/alter)
        columns: Column definitions [{"name": "id", "type": "BIGINT", "comment": "Primary key"}]
        table_type: "MANAGED" or "EXTERNAL" (default: "MANAGED")
        comment: Table description
        storage_location: Cloud URL for EXTERNAL tables
        cluster_by: Liquid clustering columns (e.g., ["region", "date"])
        table_properties: Key-value table properties
        select_query: SELECT SQL for create_as_select
        source_table: Full source table name for clone
        clone_type: "DEEP" or "SHALLOW" (default: "DEEP")
        version: Version number for clone with time travel
        timestamp: Timestamp for clone with time travel
        new_name: New table name for alter/rename
        owner: New owner for alter
        set_properties: Properties to set for alter
        add_columns: Columns to add for alter [{"name": "col", "type": "STRING"}]
    ...
    if act == "create":
        if cluster_by or table_properties:
            return _create_table_sql(
                catalog_name=catalog_name, schema_name=schema_name, table_name=table_name,
                columns=columns, table_type=table_type, comment=comment,
                storage_location=storage_location, cluster_by=cluster_by,
                table_properties=table_properties, warehouse_id=warehouse_id,
            )
        else:
            from databricks.sdk.service.catalog import ColumnInfo, TableType as TT
            col_infos = [ColumnInfo(name=c["name"], type_name=c["type"]) for c in (columns or [])]
            tt = TT.EXTERNAL if table_type.upper() == "EXTERNAL" else TT.MANAGED
            result = _create_table(
                catalog_name=catalog_name, schema_name=schema_name, table_name=table_name,
                columns=col_infos, table_type=tt, comment=comment, storage_location=storage_location,
            )
            return _to_dict(result)

    elif act == "get":
        return _to_dict(_get_table(full_table_name=full_table_name))

    elif act == "list":
        return {"items": _to_dict_list(_list_tables(catalog_name=catalog_name, schema_name=schema_name))}

    elif act == "delete":
        _delete_table(full_table_name=full_table_name)
        return {"status": "deleted", "table": full_table_name}

    elif act == "create_as_select":
        return _create_table_as_select(
            catalog_name=catalog_name, schema_name=schema_name, table_name=table_name,
            select_query=select_query, comment=comment, cluster_by=cluster_by,
            warehouse_id=warehouse_id,
        )

    elif act == "clone":
        return _clone_table(
            source_table=source_table, target_catalog=catalog_name,
            target_schema=schema_name, target_table=table_name,
            clone_type=clone_type, version=version, timestamp=timestamp,
            warehouse_id=warehouse_id,
        )

    elif act == "alter":
        return _alter_table(
            full_table_name=full_table_name, new_name=new_name, owner=owner,
            set_properties=set_properties, add_columns=add_columns,
            warehouse_id=warehouse_id,
        )
...
# =============================================================================
# Tool 7: manage_uc_security_policies
# =============================================================================
