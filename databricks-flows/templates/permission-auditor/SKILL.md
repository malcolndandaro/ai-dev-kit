...
context: fork
agent: flow:permission-auditor-agent
allowed-tools:
  - Bash
  - Read
  - Grep
...
**If $ARGUMENTS is empty, respond with:** "Usage: /flow:permission-auditor <catalog> [schema]" and stop.
...
NEXT STEPS
──────────
- Audit null/completeness      → /flow:null-completeness-auditor {catalog}.{schema}
- Monitor data freshness       → /flow:data-freshness-monitor {catalog}.{schema}
