---
name: flow:data-freshness-monitor
...
context: fork
agent: flow:data-freshness-monitor-agent
allowed-tools:
  - Bash
  - Read
  - Grep
...
**If $ARGUMENTS is empty or missing catalog.schema, respond with:** "Usage: /flow:data-freshness-monitor <catalog.schema> [table1,table2,...]" and stop.
...
NEXT STEPS
──────────
- Check job health            → /flow:job-run-monitor
- Profile stale tables        → /aidevkit:flow:data-explorer {catalog}.{schema}
- Optimize table storage      → /aidevkit:flow:table-analyzer {catalog}.{schema}
════════════════════════════════════════════════════════════
```
